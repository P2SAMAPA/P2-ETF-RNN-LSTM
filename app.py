# app.py  — P2-ETF-RNN-LSTM  (Two top-level groups with full subtabs)
...
if live_ran and live_results:
    ...
    latest_rankings_all = pd.DataFrame([
        {
            "rank":                 i + 1,
            "etf":                  r["etf"],
    ...
    latest_rankings_all = rankings[rankings["date"] == rankings["date"].max()].sort_values("rank")
    if len(latest_rankings_all) > 0:
        signal_date = pd.Timestamp(latest_rankings_all.iloc[0]["date"])
        signal_date_str = signal_date.strftime("%A %b %d, %Y")
    else:
        signal_date_str = ""
else:
...
# ══════════════════════════════════════════════════════════════════════════════
# Helper: Render full dashboard for a given ETF group
# ══════════════════════════════════════════════════════════════════════════════
def render_dashboard_for_group(group_etfs, group_name):
    """Create all subtabs (signals, forecast, consensus, performance, hurst, audit, about)
    filtered to the provided ETF list."""
    
    # Filter all dataframes to this group
    if latest_rankings_all is not None and len(latest_rankings_all) > 0:
        group_rankings = latest_rankings_all[latest_rankings_all["etf"].isin(group_etfs)].copy()
        # renumber ranks
    ...
    group_predictions = predictions[predictions["etf"].isin(group_etfs)] if predictions is not None else None
    group_rankings_all = rankings[rankings["etf"].isin(group_etfs)] if rankings is not None else None
    group_metrics = metrics_df[metrics_df["etf"].isin(group_etfs)] if metrics_df is not None else None
    group_audit = audit_df[audit_df["signal_etf"].isin(group_etfs)] if audit_df is not None else None
    ...
    with subtabs[1]:
        ...
        if group_audit is not None and len(group_audit) > 0 and ret_df is not None:
            oos = group_audit.copy()
            oos["date"] = pd.to_datetime(oos["date"])
            oos = oos.sort_values("date")

            if group_rankings_all is not None:
                rank1 = (group_rankings_all[group_rankings_all["rank"] == 1][["date", "etf"]]
    ...
    with subtabs[2]:
        ...
        if conviction_df is not None:
            conviction_df = conviction_df[conviction_df["etf"].isin(group_etfs)]
        if flat_df is not None:
            flat_df = flat_df[flat_df["etf"].isin(group_etfs)]
    ...
    with subtabs[3]:
        ...
        if (group_audit is not None and group_rankings_all is not None
                and ret_df is not None and bench_ret is not None):
            perf = compute_performance(group_audit, bench_ret, bench_col, group_rankings_all)
            if perf:
    ...
    with subtabs[4]:
        ...
        if group_metrics is not None and len(group_metrics) > 0:
            latest = group_metrics.sort_values("run_date").groupby("etf").last().reset_index()
            fig_h  = go.Figure()
    ...
    with subtabs[5]:
        ...
        if group_rankings_all is not None and len(group_rankings_all) > 0 and ret_df is not None:

            _today = pd.Timestamp(datetime.now().date())
            _last_td = last_trading_day_on_or_before(_today - timedelta(days=1))

            rank1 = (group_rankings_all[group_rankings_all["rank"] == 1]
...
# ══════════════════════════════════════════════════════════════════════════════
# TOP-LEVEL TABS: Fixed Income vs Equity
# ══════════════════════════════════════════════════════════════════════════════
main_tabs = st.tabs(["🏛️ Fixed Income ETFs", "⚡ Equity ETFs"])

with main_tabs[0]:
    render_dashboard_for_group(FI_ETFS, "Fixed Income")

with main_tabs[1]:
    render_dashboard_for_group(EQUITY_ETFS, "Equity")
